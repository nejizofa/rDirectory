$(document).ready(function() {
    ...
    $("#stateDropdown")
        .append($("<option>", { value : ""}).text('Please Select Your State'));

    $("#stateSchools").hide();

    _.each(stateProperties, function(obj){
        $("#stateDropdown")
    ...
    $("#stateDropdown").change(function(){
        ...
        _.each(stateProperties, function(obj){
            if(obj.id != thisObj.val())
            {
                return;
            }
            if(_.isEmpty(obj.schools))
            {
                $("#stateSchools")
                    .find('option')
                    .remove()
                    .end()
                    .append($("<option>", { value : ""}).text('No Schools Available'));
                return;
            }
            $("#stateSchools")
                .find('option')
                .remove()
                .end()
                .append($('<option>', {value : ""}).text('Select a School'));
            _.each(obj.schools, function(school){
                var schoolId = school.url.substring(school.url.lastIndexOf("/"));
                /*if(schoolId != 'sacramento-ca')
                {
                    $("#stateSchools").append($("<option>", { value : schoolId+"/cosmetology"}).text(school.name));
                }
                else
                {
    ...
    function renderMap(width, height)
    {
        ...
        function clicked(d) {
            ...
            if (d && centered !== d) {
                ...
                newDiv.css("right", "0");
                newDiv.css("padding-right", "20px");
                newDiv.attr("id", "stateLabel");
                var textVal = "";
                _.each(result[0].schools, function(obj){
                    var subDiv = $("<div>");
